...
const c=await p.connect(); await c.query("SET statement_timeout='160s'");
const q=async(l,sql)=>{const t=Date.now();
  try{const r=await c.query(sql);console.log(`\n■ ${l}  (${((Date.now()-t)/1000).toFixed(1)}s)`);console.table(r.rows.slice(0,15));}
  catch(e){console.log(`\n■ ${l} → ${e.message.slice(0,130)}`)}};

// 主キーワード1件だけ・直近3年に絞って軽くする
await q('跳ねやすい主キーワード（情報通信・直近3年・主キーワードのみ）', `
