...
export async function buildTurn0(companyId) {
  ...
  const facts = {
    ...
    停止期間:
      history.stopped_months !== null
        ? `${history.stopped_months}か月`
        : '不明',
    ...
    最も多く配信している企業群の当たり率: top
      ? `${top.hit_pct}%（${top.bucket}）`
      : '—',
    手応えのある結果の基準: hit
      ? `${hit.threshold_pv}PV以上（業種内の上位10%）`
      : '—',
    同じ本数で止まってから再開した企業数: seg
      ? `${seg.companies.toLocaleString()}社`
      : '—',
    再開前の当たり率: seg ? `${seg.hit_before_pct}%` : '—',
    再開後の当たり率: seg ? `${seg.hit_after_pct}%` : '—',
    再開後に追加した本数の中央値: seg ? `${seg.added_p50}本` : '—',
    '2年以上あけてから再開した企業数': longGap
      ? `${longGap.companies.toLocaleString()}社`
      : '—',
  }
  ...
  const out = {
    ...
    blocks: [
      ...
      hit && {
        ...
        evidence: {
          companies: hit.total_companies,
          axes: [company.industry_name || '業種'],
        },
      },
      seg && {
        type: 'resume',
...
function bridgeDraft(trends, best, history) {
  ...
  return (
    `${head}${common.release_type_name}の告知は、この業種で最も多く出されている形です（${common.n.toLocaleString()}本）。` +
...
export async function buildTurn1(companyId, interest) {
  ...
  const facts = {
    ...
    '3年経過時点の当たり率': period?.rows?.length
      ? `${period.rows[period.rows.length - 1].hit_pct}%`
      : '—',
    '3年経過時点の配信本数の中央値': period?.rows?.length
      ? `${period.rows[period.rows.length - 1].releases_p50}本`
      : '—',
    最も跳ねやすい種別: bestType
      ? `${bestType.release_type_name}（跳ねたとき${bestType.pv_p90}PV）`
      : '—',
