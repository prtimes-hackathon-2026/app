...
import { findStoppedCompanies, getCompany, getHitCurve, getLevers, getPeriodCurve, getTrends } from './metrics.js';
...
app.post('/api/stt', express.raw({ type: 'audio/*', limit: '25mb' }), async (req, res) => {
  try {
    const text = await listen(req.body, req.get('content-type') || 'audio/webm');
...
app.get('/api/inspect', async (_req, res) => {
  ...
  try {
    ...
    const wanted = ['company', 'release', 'release_statistic', 'webclipping_list',
      'release_keyword', 'release_type', 'industry'];
    const counts = {};
    for (const t of wanted) {
      if (!tables.some((r) => r.table_name === t)) { counts[t] = null; continue; }
      try {
...
async function warm() {
  ...
  try {
    ...
    await Promise.all([
      getHitCurve(c.industry_id), getTrends(c.industry_id),
      getPeriodCurve(c.industry_id), getLevers(c.industry_id),
    ]);
    console.log('✔ 準備完了');
...
app.listen(port, '0.0.0.0', () => {
  console.log(`▶ http://localhost:${port}  (${USING_MOCK ? 'モックデータ' : maskedUrl()})`);
  warm();
});
