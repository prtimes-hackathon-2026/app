...
await q('webclippingの件数分布（サンプル10万本）',`
 ...
 FROM c GROUP BY 1 ORDER BY 1`, 90000);

await q('PV中央値（情報通信・nth別）所要時間テスト',`
 ...
 FROM ranked WHERE nth<=8 GROUP BY nth ORDER BY nth`, 90000);
await p.end();
