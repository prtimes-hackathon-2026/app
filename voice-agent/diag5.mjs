...
const c=await p.connect(); await c.query("SET statement_timeout='150s'");
const [{industry_id:IID}] = (await c.query(
  "SELECT industry_id FROM industry WHERE industry_name='情報通信'")).rows;
console.log('情報通信 industry_id =', IID);
const q=async(l,sql)=>{const t=Date.now();
  try{const r=await c.query(sql,[IID]);
    console.log(`\n■ ${l}  (${((Date.now()-t)/1000).toFixed(1)}s)`);console.table(r.rows.slice(0,12));}
  catch(e){console.log(`\n■ ${l} → ${e.message.slice(0,100)}`)}};

await q('配信本数別「当たり(PV上位10%)を引いた企業の割合」',`
