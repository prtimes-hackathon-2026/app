...
// 出力に必ず残っていなければいけない数値
const MUST = { position: ['1', '17'], lottery: ['87'], resume: ['4,291', '44', '3'] };
...
for (const model of models) {
  ...
  const { narrate } = await import(`./src/narrate.js?m=${encodeURIComponent(model)}`);
  const t = Date.now();
  ...
  for (const [k, v] of Object.entries(text)) if (k !== 'source') console.log(`  [${k}] ${v}`);
