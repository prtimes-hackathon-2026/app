...
for (const m of ['gpt-4o-mini', 'gpt-3.5-turbo']) {
  ...
  const u = r.usage || {}
  console.log(
    `${m.padEnd(16)} 入力 ${u.inputTokens ?? u.promptTokens} / 出力 ${u.outputTokens ?? u.completionTokens}`,
  )
}
...
for (const s of sentences) {
  ...
  if (!r.ok) {
    console.log('  TTS失敗', r.status, (await r.text()).slice(0, 120))
    break
  }
  ...
  console.log(
    `  ${s.length}文字 → ${(b / 1024).toFixed(0)}KB  約${(b / 4000).toFixed(1)}秒`,
  )
}
console.log(
...
console.log(
  `ターン0の台本は約5文なので、概算で ${(chars / 2) * 5}文字・${((bytes / 4000 / 2) * 5).toFixed(0)}秒程度`,
)
