...
どちらもログインが要る (下の「簡易ログイン」)。

## 簡易ログイン

合言葉を 1 つ知っていれば入れる、というだけのログイン。利用者アカウントも権限も無い。

1. `/login` でパスワードを入力する (既定は `prtimes`)
2. `/login/company` で企業を 1 社選ぶ
3. 選んだ企業として管理画面を使う。PR羅針盤 の対話もこの企業のデータで進む
...
| 変数                  | 既定             | 備考                                               |
| --------------------- | ---------------- | -------------------------------------------------- |
| `AUTH_PASSWORD`       | `prtimes`        | 合言葉。公開する環境では必ず差し替える             |
| `AUTH_SESSION_SECRET` | (開発用の固定値) | Cookie の署名鍵。production では必須 (32 文字以上) |

`AUTH_SESSION_SECRET` が未設定のまま `NODE_ENV=production` で起動すると、
ログインを扱う最初のリクエストで例外になる。`openssl rand -base64 32` で作って渡す。
