...
1. `/login` で「企業利用者」または「管理者」を選び、パスワードを入力する
2. 企業利用者は `/login/company` で企業を 1 社選ぶ
3. 管理者は企業を選ばず、営業フロー事例の管理画面へ進む

既定の企業パスワードは `prtimes`、管理者パスワードは `admin`。企業利用者には
営業フロー事例のメニューを表示せず、URLや保存処理へ直接アクセスしても管理者セッションを要求する。
