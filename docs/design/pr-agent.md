...
**解消済み。** 共有パスワード 1 つの簡易ログイン（`/login` → 企業の選択）を入れ、
企業IDはセッションから決まるようになった。会話を開始する `POST /api/pr-agent/conversations`
は企業IDを受け取らず、他社の会話IDを渡しても 404 になる。
予定どおり差し替えたのは `app` 層だけで、`feature` 側の契約は変えていない。
仕組みと、認証ライブラリを使わなかった理由は README の「簡易ログイン」にある。

利用者ごとのアカウント・権限・監査は引き続き無い。必要になったら
`src/feature/auth/` の中身を差し替える（画面と Route Handler が触るのは
`src/app/session.ts` だけなので、外側は変わらない）。
